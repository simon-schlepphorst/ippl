...
public:
    ...
    static constexpr T tol() { return std::numeric_limits<T>::epsilon() * 10.0; }
    ...
    static field_t make_zero_field(Mesh_t& mesh, ippl::FieldLayout<dim>& layout) {
        field_t f;
    ...
    static void set_field_from_function(field_t& coeffs, const Func& u) {
        ...
        for (unsigned d = 0; d < D; ++d)
            ext[d] = last[d] - first[d] + 1;

        ippl::Vector<int, D> size;  // ext + 2*nghost per axis
        for (unsigned d = 0; d < D; ++d)
            size[d] = ext[d] + 2 * nghost;

        if constexpr (D == 1) {
            Kokkos::parallel_for(
                "fill_field_fun_1d", Kokkos::RangePolicy<exec_space>(0, size[0]),
            ...
            Kokkos::parallel_for(
                "fill_field_fun_3d",
                Kokkos::MDRangePolicy<Kokkos::Rank<3>, exec_space>({0, 0, 0},
    ...
    static void fill_uniform_positions(bunch_t& bunch, int n_local, const Mesh_t& M,
                                       uint64_t seed = 4242) {
    ...
    static void domain_bounds(const Mesh_t& M, ippl::Vector<T, dim>& xmin,
...
void InterpolatesAffineExactly() {
    ...
    EXPECT_LE(g_err, allow)
        ...
        << "  allowed (abs+rel)       = " << allow << " (abs_tol=" << abs_tol
        << ", rel_tol*ref=" << rel_tol * g_ref << ")";
